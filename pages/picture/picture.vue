<script>
	...
	export default {
		...
		methods: {
			...
			getAllTypes() {
				uni.request({
					url: urlServer.ApiUrl + '/categories/Girl',
					success: res => {
			...
			getList() {
				...
				uni.request({
					url: `${urlServer.ApiUrl}/data/category/Girl/type/${this.selected}/page/${this.page}/count/10`,
					success: res => {
						if (res.data.status === 100) {
...
<template>
	<view class="class-transition-content">
		...
		<ad class="ad_pic" unit-id="adunit-0edab946252335bf"></ad>
		<scroll-view class="content_wrap img_wrap" scroll-y @scrolltolower="loadBottom" refresher-enabled enable-back-to-top
		 @refresherrefresh="refreshList" :refresher-triggered="freshing">
			<detail-list :contenttype="'img'" :contentlist="contentList" />
		</scroll-view>
	</view>
</template>
<style lang="scss">
